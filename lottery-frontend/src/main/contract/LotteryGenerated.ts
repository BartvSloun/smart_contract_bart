...
export function newLotteryState(
  signedBy: BlockchainAddress[],
  description: string
): LotteryState {
...
function fromScValueLotteryState(structValue: ScValueStruct): LotteryState {
  return {
...
export function deserializeLotteryState(state: StateBytes): LotteryState {
  const scValue = new StateReader(state.state, fileAbi.contract, state.avlTrees).readState();
  return fromScValueLotteryState(scValue);
}
